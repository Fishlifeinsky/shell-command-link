/**
  ******************************************************************************
  * @file    scl_exec.c
  * @brief   SCL 编译链与字节码执行核（含 help 与内置命令实现）
  *
  *          由 scl.c 拆出（模块化）。**编译链与执行器刻意放在同一个编译单元**：
  *          它们共享参数字节缓存与一批 static 读回原语
  *          （Scl_ArgLoad/Scl_BlkText/Scl_ArgRestore/Scl_DoCallName），
  *          而这些原语在 mini 态仍会被编译；一旦为了跨文件而外置，
  *          就再也不能被"未引用即消除"（详见 doc/idea/scl-module-split.md §10.3/§11）。
  *
  *          本文件内容：
  *            - opcode 常量（内建指令；业务命令 opcode 见 scl_cmd.c 的注册表）
  *            - 编译态与执行态静态缓冲（随本模块走，整段包在 RUN_TEXT||RUN_PROG 内）
  *            - 文本 → 字节码（token 化 / label / 参数字节缓存）
  *            - 字节码解释执行（Step/分支/算子）与收尾
  *            - 内置命令实现（var / free / cache / help）
  *            - SCL_Run / SCL_RunProg 与执行核初始化/释放/复位接口
  *
  *          与迁移前逐行一致（行为零变化）。
  ******************************************************************************
  */
...
/* ========================== 执行态（编译 + 执行，随本模块走） ==========================
   整段包在 RUN_TEXT || RUN_PROG 内：mini 态不含本文件任何执行态，
   由 Scl_ExecInit/Release/Reset 三个接口驱动（见文件末尾）。 */
...
/* ---- 元指令参数原文缓冲（Scl_ArgLoad 用）与 CALLN 命令名缓冲（Scl_DoCallName 用）----
   二者属"执行期读回"，在 RUN_PROG（const 程序）路径下**同样需要**，
   故按 RUN_TEXT || RUN_PROG 编译（而不是只在文本链下）；只被本模块引用，保持 static。 */
#if (SCL_CFG_DYNAMIC_MEM_EN != 0u)
static char *s_raw = NULL;
static char *s_cmdname = NULL;
#else
static char s_raw[SCL_RAW_MAX];
static char s_cmdname[SCL_CMDNAME_MAX];
#endif

#endif /* RUN_TEXT || RUN_PROG */
...
/* 前向：定义在本文件靠后的 ${} 展开拷贝（STR 参数块读回时要展开） */
static int Scl_ExpandCopy(const char *src, const char *end, char *dst, uint16_t cap);
...
/* 按 opcode 查找命令（解释执行用；只被本模块引用，故保持 static） */
...
/* 成功 0；-1 变量名过长；-2 目标缓冲不足（已尽量写入）。
   只被本模块引用（STR 参数块读回、var 指令取值、脚本命令参数注入），
   故保持 static，并按 RUN_TEXT || RUN_PROG 编译（mini 态无任何引用者）。 */
#if ((SCL_CFG_RUN_TEXT_EN) != 0u) || ((SCL_CFG_RUN_PROG_EN) != 0u)
...
/* ========================== 内置命令：help / var / free / cache ==========================
   注意：这些元指令在 **const 预编译程序**里同样会出现（scl_emit_c 生成的程序尾部
   就带一条 free 指令），故整段按 RUN_TEXT || RUN_PROG 编译，而不是只在文本链下编译。 */
...
/* G_RETURN 取值并清零（jump -a 用；只被本模块引用，故保持 static） */
...
/* ========================== 执行核缓冲的初始化 / 释放 / 复位 ==========================
   由 scl.c 的 SCL_InitEx 驱动（分配/释放"跟着使用它的模块走"，mini 态整段不参与）。 */
...
/* ========================== 主循环（普通态） ==========================
   放在本文件而不是 scl.c：它们要直接操作执行态，并调用同样是 static 的 Scl_Finish。 */
...
#else   /* mini 态：无解释器，接口给空实现，调用方无需条件编译 */

uint8_t Scl_ExecInit(void)     { return 1u; }
void    Scl_ExecRelease(void)  { }
void    Scl_ExecReset(void)    { }
...
/* ============================ 脚本命令（s2c 编译产物注册成命令） ============================
   放在执行核的原因：SCL_Scmd_RunText 要用执行的 ${} 展开与 SCL_RunProg；mini 态整段不编译。 */

#if ((SCL_CFG_SCMD_EN != 0u) && (SCL_CFG_RUN_PROG_EN != 0u))

scl_scmd_t *s_scmd_head = NULL;          /* 脚本命令链表头（SCL_InitEx 会复位；声明见 scl_priv.h） */
