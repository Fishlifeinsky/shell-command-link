...
#include "boot_mini.c"      /* 生成的状态机：boot_mini_start / boot_mini_step */
...
int main(void)
{
    ...
    CHECK(guard < 200000, "有限步数内完成（无死循环）");
    {
        const char *s = SCL_VarGet("sum");
        CHECK(s != NULL && strcmp(s, "3") == 0, "外部 SCL_VarGet(sum)==3（镜像）");
        s = SCL_VarGet("i");
        CHECK(s != NULL && strcmp(s, "3") == 0, "外部 SCL_VarGet(i)==3");
    }
