...
> 状态：**已闭环**（步骤 1~5 全部完成：`0c6a49b` / `950d94b` / `5d0e03e` / `e8cac3f` / `137607e`）
> 场景分类：运行时/解释器 ＋ 构建工具
...
- [x] 回归：`tools/scl_build.py all` 全绿（每步；含 7 组裁剪矩阵与 8 档尺寸）
- [x] 体积：前后实测，Flash 变化 ≤ ±2%（超出已在 §10 解释）
- [x] 文档：`doc/arc/scl-modules.md` 增模块划分、依赖方向、裁剪边界与共享状态表
- [x] 提交：`change:` 分批提交（每步代码 + 文档各一次）
- [x] 回填：本文件补数据与提交号（§10.1~§10.5）
...
| 2026-09-10 | 步骤 5：抽出 `scl_exec.c`（编译链 + 执行核 + 内置命令 + 脚本命令） | 全绿；`scl.c` 收敛为 330 行；体积见 §10.5 | `137607e` |

### 10.5 步骤 5：抽出 `scl_exec.c`（编译链 + 执行核）

| 档位 | 步骤 4 | 步骤 5 | 变化 |
|---|---|---|---|
| 默认全功能 O2 | 19919 | 20079 | +160（+0.8%） |
| 默认-关绑定表 O2 | 19703 | 19863 | +160 |
| 默认 -Os | 17126 | 17242 | +116 |
| 档A 最小RAM O2 | 16761 | 17001 | +240（+1.4%） |
| 档A+关消息 O2 | 9511 | 9659 | +148 |
| 档B 平衡 O2 | 19947 | 20115 | +168 |
| mini O2 | 4774 | 4798 | +24（+0.5%） |
| mini -Os | 3684 | 3696 | +12 |
| RAM（默认档） | 2013 | 2017 | +4 |

拆分后 `scl.c` 收敛到 **330 行**、`scl_exec.c` **2187 行**。
跨 TU 后原本在同一单元内自动内联的短函数（`Scl_Finish`/`Scl_RetTake`/参数缓存原语等）
变成真实调用，这是 **+12~240 B 的固定代价**；RAM 的 +4 B 来自 `s_scmd_head` 归属变化与对齐。

**拆分全程累计（拆分起点 `dd3721c` → 现在）**：

| 档位 | 拆分前 | 现在 | 变化 |
|---|---|---|---|
| 默认全功能 O2 | 21362 | 20079 | **−1283（−6.0%）** |
| 档A 最小RAM O2 | 17820 | 17001 | −819 |
| 档A+关消息 O2 | 10199 | 9659 | −540 |
| mini O2 | 4954 | 4798 | **−156（−3.1%）** |
| RAM（默认档） | 2014 | 2017 | +3 |

> 体积整体是变小方向：原单 TU 里变参 `Scl_Msg` 被内联到数十个调用点造成的膨胀
> 远大于跨 TU 调用开销。模块划分的收益体现在**代码结构与裁剪边界**上，
> 而非单看某一档的几个字节。
